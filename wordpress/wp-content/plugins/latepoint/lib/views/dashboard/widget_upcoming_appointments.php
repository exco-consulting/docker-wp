		...
		<div class="os-widget-header-actions">
			...
			<select name="agent_id" id="" class="os-trigger-reload-widget">
				...
				if($agents){
					...
					<option value="<?php echo $agent->id ?>" <?php if($agent->id == $selected_agent_id) echo 'selected="selected"' ?>><?php echo esc_html($agent->full_name); ?></option>
					<?php }
				} ?>
			</select>
			<?php } ?>
	...
	<div class="os-widget-content no-padding">
		...
		<?php }else{ ?>
		  <div class="no-results-w">
		    ...
		    <a href="#" <?php echo OsOrdersHelper::quick_order_btn_html(); ?> class="latepoint-btn latepoint-btn-link"><i class="latepoint-icon latepoint-icon-plus"></i><span><?php _e('Add Appointment', 'latepoint'); ?></span></a>
		  </div>
		<?php } ?>
		</div>
	</div>
