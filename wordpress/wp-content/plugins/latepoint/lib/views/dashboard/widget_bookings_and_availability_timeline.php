<div class="os-widget os-widget-agents-bookings-timeline os-widget-animated" data-os-reload-action="<?php echo OsRouterHelper::build_route_name('dashboard', 'widget_bookings_and_availability_timeline'); ?>">
	<div class="os-widget-header with-actions">
		<h3 class="os-widget-header-text"><?php _e('Day Preview', 'latepoint'); ?></h3>
		...
		<div class="os-widget-header-actions">
			<div class="os-date-range-picker" data-single-date="yes">
				<span class="range-picker-value"><?php echo OsTimeHelper::get_readable_date($target_date_obj); ?></span>
				<i class="latepoint-icon latepoint-icon-chevron-down"></i>
				<input type="hidden" name="date_from" value="<?php echo $target_date_obj->format('Y-m-d'); ?>"/>
				<input type="hidden" name="date_to" value="<?php echo $target_date_obj->format('Y-m-d'); ?>"/>
			</div>
			...
			<select name="service_id" id="" class="os-trigger-reload-widget">
				<?php echo '<option value="">'.__('All Services', 'latepoint').'</option>'; ?>
			...
			<div class="timeline-type-toggle" data-value-holder-id="what_to_show_on_timeline_widget">
				<div class="timeline-type-option <?php echo ($what_to_show == 'appointments') ? 'active' : ''; ?>" data-value="appointments"><?php _e('Show Appointments', 'latepoint'); ?></div>
				<div class="timeline-type-option <?php echo ($what_to_show == 'availability') ? 'active' : ''; ?>" data-value="availability"><?php _e('Show Availability', 'latepoint'); ?></div>
			</div>
		</div>
	</div>
		...
		<?php if($agents){ ?>
			<div class="timeline-with-info-w">
				<div class="timeline-and-availability-w">
						<div class="timeline-and-availability-contents shows-<?php echo $what_to_show; ?>">
							<div class="agent-day-bookings-timeline-compact-w">
								<div class="agents-avatars">
										<?php foreach($agents as $agent){ ?>
											<div class="avatar-w">
												<a href="<?php echo OsRouterHelper::build_link(['agents', 'edit_form'], ['id' => $agent->id] ) ?>" class="avatar-i" style="background-image: url(<?php echo $agent->get_avatar_url(); ?>);">
								...
								<div class="agents-timelines-w">
									<?php if($show_today_indicator) echo '<div class="current-time-indicator" style="left: '.$time_now_indicator_left_offset.'%"><span>'.$time_now_label.'</span></div>'; ?>
									<div class="timeline-top-w">
										<?php
										for($minutes = $work_boundaries->start_time; $minutes <= $work_boundaries->end_time; $minutes+= 5){
											if(($minutes % 60) == 0){
												echo '<div class="timeslot with-tick"><div class="tick"></div><div class="timeslot-time"><div class="timeslot-hour">'.OsTimeHelper::minutes_to_hours($minutes).'</div><div class="timeslot-ampm">'.OsTimeHelper::am_or_pm($minutes).'</div></div></div>';
											}else{
													...
													if($bookings && $work_total_minutes){
														...
														foreach($bookings as $index => $booking){
															...
															if(OsBookingHelper::check_if_group_bookings($booking, $next_booking)){
																...
																echo '<div data-start="'.$booking->start_time.'" data-end="'.$booking->end_time.'" class="booking-block status-'.$booking->status.'" '.$action_html.' style="background-color: '.$booking->service->bg_color.'; left: '.$left.'%; width: '.$width.'%;'.$custom_height.'">';
									...
									<?php if(count($agents) > 3){ ?>
										<div class="timeline-bottom-w">
											...
											for($minutes = $work_boundaries->start_time; $minutes <= $work_boundaries->end_time; $minutes+= 5){
												if(($minutes % 60) == 0){
													echo '<div class="timeslot with-tick"><div class="timeslot-time"><div class="timeslot-hour">'.OsTimeHelper::minutes_to_hours($minutes).'</div><div class="timeslot-ampm">'.OsTimeHelper::am_or_pm($minutes).'</div></div></div>';
												}else{
													echo '<div class="timeslot"></div>';
												}
											}
