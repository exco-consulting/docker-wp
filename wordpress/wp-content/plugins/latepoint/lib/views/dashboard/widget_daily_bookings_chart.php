		...
		<div class="os-widget-header-actions">
			...
			<select name="agent_id" id="" class="os-trigger-reload-widget" style="<?php echo $agents_selector_css; ?>">
				...
				if($agents){
					...
					<option value="<?php echo $agent->id ?>" <?php if($agent->id == $filter->agent_id) echo 'selected="selected"' ?>><?php echo esc_html($agent->full_name); ?></option>
					<?php }
				} ?>
			</select>
					...
					<div class="stats-line-chart-w">
						...
						<?php }else{ ?>
						  <div class="no-results-w">
						    ...
						    <a href="#" <?php echo OsOrdersHelper::quick_order_btn_html(); ?> class="latepoint-btn"><i class="latepoint-icon latepoint-icon-plus-square"></i><span><?php _e('Create Appointment', 'latepoint'); ?></span></a>
						  </div>
						<?php } ?>
					</div>
					<?php
