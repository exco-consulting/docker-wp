...
if($agents){ ?>
	...
	echo '<div class="calendar-month-agents-w '.$calendar_not_scrollable_class.'" data-route="'.OsRouterHelper::build_route_name('calendars', 'month_view').'">';
	    ...
	    for($day_date=clone $calendar_start; $day_date<=$calendar_end; $day_date->modify('+1 day')){
				echo '<div class="ma-day ma-day-number-'.$day_date->format('N').' '.(($today_date == $day_date) ? 'is-today' : '').'">';
					echo '<div class="ma-day-info">';
						echo '<span class="ma-day-number">'.$day_date->format('j').'</span>';
						echo '<span class="ma-day-weekday">'.OsUtilHelper::get_weekday_name_by_number($day_date->format('N'), true).'</span>';
					echo '</div>';
				echo '</div>';
			}
		echo '</div>';
							...
							if($day_periods){
								echo '<div class="ma-day-agent-bookings" '.OsOrdersHelper::quick_order_btn_html(false, array('agent_id' => $agent->id, 'location_id' => $booking_request->location_id, 'start_date' => $day_date->format('Y-m-d'))).'>';
									...
									echo '<div class="ma-day-work-periods">';
										echo '<div class="ma-day-label">'.$day_date->format($date_format).': </div>';
										foreach($day_periods as $day_period){
											echo '<div class="ma-day-work-period">';
												echo OsTimeHelper::minutes_to_hours_and_minutes($day_period->start_time).' - '.OsTimeHelper::minutes_to_hours_and_minutes($day_period->end_time);
											echo '</div>';
										}
									echo '</div>';
								...
								echo '<div class="ma-day-agent-bookings is-day-off">';
									echo '<div class="ma-day-off full"><span><strong>'.$day_date->format($date_format).': </strong>'.__('Day Off', 'latepoint').'</span></div>';
								echo '</div>';
							}
						}
					echo '</div>';
