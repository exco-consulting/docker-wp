...
<div class="daily-availability-calendar-wrapper">
	<a href="#" data-target-date="<?php echo (clone $target_date)->modify('last day of previous month')->format('Y-m-d'); ?>" class="daily-calendar-action-navigation-btn"><i class="latepoint-icon latepoint-icon-chevron-left"></i></a>
	...
	<a href="#" data-target-date="<?php echo (clone $target_date)->modify('first day of next month')->format('Y-m-d'); ?>" class="daily-calendar-action-navigation-btn"><i class="latepoint-icon latepoint-icon-chevron-right"></i></a>
</div>
...
<div class="daily-agent-calendar-w <?php if(count($agents) > 5) echo 'make-scrollable'; ?>">
	<?php if($show_today_indicator) echo '<div class="current-time-indicator" style="top: '.$time_now_indicator_top_offset.'px"><span>'.$time_now_label.'</span></div>'; ?>
	<div class="calendar-daily-agent-w">
		<?php if($valid_work_hours){ ?>
			...
			<div class="calendar-hours">
				<div class="ch-hours">
					<div class="ch-filter">
						<span><?php _e('Agent', 'latepoint'); ?></span>
					...
					<?php for($minutes = $work_boundaries->start_time; $minutes <= $work_boundaries->end_time; $minutes+= $timeblock_interval){ ?>
						...
						echo '<div class="'.$period_class.'" style="'.$period_css.'"><span>'.OsTimeHelper::minutes_to_hours_and_minutes($minutes, false, true, true).'</span></div>';
						?>
					<?php } ?>
				</div>
				<div class="ch-agents">
				  ...
				  <?php foreach($agents as $agent){ ?>
						<div class="da-head-agent">
							...
							<a href="<?php echo OsRouterHelper::build_link(['agents', 'edit_form'], ['id' => $agent->id]); ?>" class="da-head-agent-name"><?php echo esc_html($agent->full_name); ?></a>
						</div>
					<?php } ?>
					</div>
		  ...
		  <div class="no-results-w">
		    ...
		    <h2><?php _e('You have not set any working hours for this day.', 'latepoint'); ?></h2>
			  <?php if(OsAuthHelper::is_agent_logged_in()){ ?>
			    <a href="<?php echo OsRouterHelper::build_link(['agents', 'edit_form'], ['id' => OsAuthHelper::get_logged_in_agent_id()]); ?>" class="latepoint-btn"><i class="latepoint-icon latepoint-icon-edit-2"></i><span><?php _e('Edit Working Hours', 'latepoint'); ?></span></a>
				<?php }elseif(OsRolesHelper::can_user('settings__edit')){ ?>
			    <a href="<?php echo OsRouterHelper::build_link(['settings', 'work_periods']); ?>" class="latepoint-btn"><i class="latepoint-icon latepoint-icon-edit-2"></i><span><?php _e('Edit Working Hours', 'latepoint'); ?></span></a>
				<?php } ?>
		  </div>
		<?php } ?>
	</div>
