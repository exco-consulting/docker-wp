 ...
 **/ ?>
	...
	<div class="os-form-w quick-customer-form-w <?php echo ($customer->is_new_record()) ? 'is-new-customer' : 'is-existing-customer' ;?>" data-refresh-route-name="<?php echo OsRouterHelper::build_route_name('customers', 'quick_edit'); ?>">
		<form action=""
			    data-route-name="<?php echo ($customer->is_new_record()) ? OsRouterHelper::build_route_name('customers', 'create') : OsRouterHelper::build_route_name('customers', 'update'); ?>"
			    class="customer-quick-edit-form">
			<div class="os-form-header">
	      <?php if($customer->is_new_record()){ ?>
	        <h2><?php _e('New Customer', 'latepoint'); ?></h2>
	      <?php }else{ ?>
	        <h2><?php _e('Edit Customer', 'latepoint'); ?></h2>
	      <?php } ?>
	    ...
	    <div class="os-form-content">
		    ...
		    <div class="quick-booking-info">
			    <?php echo '<span>'.__('Customer ID:', 'latepoint').'</span><strong>'. $customer->id.'</strong>'; ?>
			    <?php if (OsAuthHelper::get_current_user()->has_capability('activity__view')) echo '<a href="#" data-customer-id="' . $customer->id . '" data-route="' . OsRouterHelper::build_route_name('customers', 'view_customer_log') . '" class="quick-customer-form-view-log-btn"><i class="latepoint-icon latepoint-icon-clock"></i>' . __('History', 'latepoint') . '</a>'; ?>
		    </div>
		    <?php } ?>
					...
					<div class="os-form-sub-header">
						<h3><?php _e('Contact Info', 'latepoint'); ?></h3>
					</div>
					<div class="os-row">
		      ...
		      <?php if(!$customer->is_new_record()){ ?>
				    <div class="customer-password-info <?php echo ($customer->is_guest) ? 'password-not-set' : 'password-set'; ?>">
					    <?php if($customer->is_guest) {
						    _e('Guest Account', 'latepoint');
					    }else{
								_e('Password Protected', 'latepoint');
								echo '<a href="#" data-os-success-action="reload" 
															    ...
															    data-os-action="'.OsRouterHelper::build_route_name('customers', 'set_as_guest').'">'.__('Convert to Guest', 'latepoint').'</a>';
							} ?>
				    </div>
					<?php } ?>
					<?php if(!$customer->is_new_record() && OsAuthHelper::wp_users_as_customers()){ ?>
					    ...
					    if($customer->wordpress_user_id){
					      echo '<div class="connected-note">'.__('Connected', 'latepoint').'</div>';
								echo '<div class="connected-buttons"> <a target="_blank" href="'.esc_attr(get_edit_user_link($customer->wordpress_user_id)).'"><i class="latepoint-icon latepoint-icon-external-link"></i><span>'.__('View Profile', 'latepoint').'</span></a> <span>or</span> <a href="#" data-os-success-action="reload" data-os-params="'.OsUtilHelper::build_os_params(['customer_id' => $customer->id]).'" data-os-action="'.OsRouterHelper::build_route_name('customers', 'disconnect_from_wp_user').'"><i class="latepoint-icon latepoint-icon-slash"></i><span>'.__('Disconnect', 'latepoint').'</a></div>';
					    }else{
					      echo '<div class="connected-note">'.__('Not Connected', 'latepoint').'</div>';
								echo '<div class="connected-buttons"> <a href="#" data-os-success-action="reload" data-os-params="'.OsUtilHelper::build_os_params(['customer_id' => $customer->id]).'" data-os-action="'.OsRouterHelper::build_route_name('customers', 'connect_to_wp_user').'"><i class="latepoint-icon latepoint-icon-link-2"></i><span>'.__('Connect', 'latepoint').'</span></a></div>';
					    }
					    ?>
					  </div>
					<?php } ?>
			...
			<?php if(!$customer->is_new_record()){ ?>
				<div class="customer-appointments">
					<div class="os-form-sub-header">
						<h3><?php _e('Appointments', 'latepoint'); ?></h3>
              ...
              <i class="latepoint-icon latepoint-icon-plus"></i><span><?php _e('Add', 'latepoint'); ?></span>
	            </a>
						</div>
					</div>
					<?php
					...
					if($customer_bookings){
						...
						foreach($customer_bookings as $booking){
					      ...
					      <div class="avatar-w" style="background-image: url(<?php echo $booking->agent->get_avatar_url(); ?>);">
					        <div class="agent-info-tooltip"><?php echo $booking->agent->full_name; ?></div>
								...
								<div class="oi-info">
									<div class="oi-service-name"><?php echo $booking->service->name; ?></div>
									<div class="oi-date-w">
										<div class="oi-date-i">
											<span class="oi-date"><?php echo $booking->nice_start_date; ?></span>, <span class="appointment-time"><?php echo implode('-', array($booking->nice_start_time, $booking->nice_end_time)); ?></span>
										</div>
									</div>
									</div>
							</div>
					  ...
					  <div class="no-results-w">
						  <?php _e('Customer does not have any bookings', 'latepoint'); ?>
					  </div>
						<?php
					} ?>
				</div>
	    ...
	    <?php
	      if($customer->is_new_record()){
	        if(OsRolesHelper::can_user('customer__create')) echo '<button name="submit" type="submit" class="latepoint-btn latepoint-btn-block latepoint-btn-lg">'.__('Create Customer', 'latepoint').'</button>';
	      }else{
	        if(OsRolesHelper::can_user('customer__edit')){
						echo '<div class="os-full">';
							echo '<button name="submit" type="submit" class="latepoint-btn latepoint-btn-block latepoint-btn-lg">'.__('Save Changes', 'latepoint').'</button>';
						echo '</div>';
	        }
	        if(OsRolesHelper::can_user('customer__delete')) {
		        ...
		        echo '<a href="#" class=" remove-customer-btn latepoint-btn latepoint-btn-secondary latepoint-btn-lg latepoint-btn-just-icon" 
	                ...
	                data-os-params="' . OsUtilHelper::build_os_params(['id' => $customer->id], 'destroy_customer_'.$customer->id) . '" 
	                data-os-success-action="redirect" 
	                data-os-action="' . OsRouterHelper::build_route_name('customers', 'destroy') . '"
	                title="' . __('Delete Customer', 'latepoint') . '">
