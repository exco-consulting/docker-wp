<div class="os-form-w">
	<form action="" data-os-output-target=".os-wizard-step-content-i" data-os-pass-response="yes" data-os-after-call="latepoint_wizard_item_editing_cancelled" data-os-action="<?php echo OsRouterHelper::build_route_name('wizard', 'save_agent'); ?>">
		<div class="os-row">
			<div class="os-col-6">
    ...
    <div class="side-by-side-buttons">
	    <?php if($agents){ ?>
			      <button type="button" data-os-after-call="latepoint_wizard_item_editing_cancelled" data-os-pass-response="yes" data-os-output-target=".os-wizard-step-content" data-os-params="current_step_code=agents" data-os-action="<?php echo OsRouterHelper::build_route_name('wizard', 'load_step'); ?>" class="wizard-finished-editing-trigger latepoint-btn latepoint-btn-lg latepoint-btn-secondary">
	            <i class="latepoint-icon latepoint-icon-arrow-left"></i>
				      <span><?php _e('Cancel', 'latepoint'); ?></span>
			      </button>
	    <?php }else{ ?>
				<a href="#" data-route-name="<?php echo OsRouterHelper::build_route_name('wizard', 'prev_step'); ?>" class="latepoint-btn latepoint-btn-lg latepoint-btn-secondary os-wizard-trigger-prev-btn"><i class="latepoint-icon latepoint-icon-arrow-left"></i> <span><?php _e('Back', 'latepoint'); ?></span></a>
	    <?php } ?>
	      <button type="submit" class="latepoint-btn latepoint-btn-lg latepoint-btn-primary">
	        <span><?php echo ($agent->is_new_record()) ? __('Save', 'latepoint') : __('Save', 'latepoint'); ?></span>
