<div class="service-boxes">
	...
	foreach ( $services as $service ) { ?>
        <div class="service-box-w wizard-add-edit-item-trigger" data-id="<?php echo $service->id; ?>"
             data-route="<?php echo OsRouterHelper::build_route_name( 'wizard', 'add_or_edit_service' ) ?>">
			<?php if ( $service->selection_image_id ) { ?>
                <div class="service-image"
                     style="background-image: url(<?php echo $service->selection_image_url; ?>)"></div>
			<?php } else { ?>
                ...
                <div class="agents-avatars">
					<?php foreach ( $service->agents as $index => $agent ) {
                        ...
                        <div class="agent-avatar"
                             style="z-index: <?php echo 3 - $index; ?>;background-image: url(<?php echo esc_url( $agent->avatar_url ); ?>)"></div>
					<?php } ?>
					<?php if ( count( $service->agents ) > 3 ) {
						echo '<div class="agents-more">+' . ( count( $service->agents ) - 3 ) . ' ' . __( 'more', 'latepoint' ) . '</div>';
					} ?>
                </div>
                <?php } ?>
            </div>
            ...
            <div class="service-remove-trigger"
                 ...
                 data-os-params="<?php echo OsUtilHelper::build_os_params( [ 'id' => $service->id ], 'destroy_service_' . $service->id ); ?>"
    ...
    <div class="add-service-box wizard-add-edit-item-trigger"
        ...
        <div class="add-service-label"><?php _e( 'Add Another Service', 'latepoint' ); ?></div>
    </div>
</div>