...
if ($transactions) {
	foreach ($transactions as $transaction) { ?>
		<tr>
			...
			<td>
				<?php if ($transaction->order_id) { ?>
					<?php echo '<a href="#" class="in-table-link" ' . OsOrdersHelper::quick_order_btn_html($transaction->order_id) . '><span>' . $transaction->order_id . '</span><span>'.esc_html__('View', 'latepoint').'</span><i class="latepoint-icon latepoint-icon-external-link"></i></a>'; ?>
				<?php } else {
					echo 'n/a';
				} ?>
			</td>
        ...
        <a class="os-with-avatar" target="_blank" <?php echo OsCustomerHelper::quick_customer_btn_html($transaction->customer->id); ?> href="#">
          ...
          <span class="os-name"><?php esc_html_e($transaction->customer->full_name); ?></span>
