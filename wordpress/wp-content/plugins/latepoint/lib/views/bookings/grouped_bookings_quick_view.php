<div class="latepoint-lightbox-heading">
	<h2><?php _e('Group Appointment', 'latepoint'); ?></h2>
</div>
<div class="latepoint-lightbox-content">
	<div class="grouped-bookings-main-info">
		...
		<div class="gb-info">
			...
			<div class="gbi-high"><?php echo OsTimeHelper::minutes_to_hours_and_minutes($booking->start_time); ?>
				- <?php echo OsTimeHelper::minutes_to_hours_and_minutes($booking->end_time); ?></div>
		</div>
		<div class="gb-capacity">
			<div
				class="gbc-label"><?php echo __('Booked:', 'latepoint') . ' <span>' . sprintf(__('%d of %d', 'latepoint'), $total_attendees, $booking->service->capacity_max) . '<span>'; ?></div>
			<div class="booked-percentage">
				<div class="booked-bar"
				     style="width: <?php echo OsServiceHelper::get_percent_of_capacity_booked($booking->service, $total_attendees); ?>%;"></div>
			</div>
		</div>
	</div>
		...
		<?php foreach ($group_bookings as $group_booking) { ?>
			<div class="gb-booking" <?php echo OsBookingHelper::quick_booking_btn_html($group_booking->id); ?>>
				...
				<div class="gbb-attendees">
					<div class="gb-value"><?php echo $group_booking->total_attendees; ?></div>
					<div class="gb-label">
						<?php echo _n( 'Attendee', 'Attendees', $group_booking->total_attendees, 'latepoint' ); ?>
					</div>
				</div>
			</div>
		<?php } ?>
	...
	<div class="add-booking-to-group-box-wrapper">
		<div
			...
			<div class="add-box-label"><?php _e('Add Booking', 'latepoint'); ?></div>
		</div>
	</div>
</div>
