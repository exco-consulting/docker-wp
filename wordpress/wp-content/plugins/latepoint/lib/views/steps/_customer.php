 ...
 */
?>
<div class="step-customer-w latepoint-step-content" data-step-code="<?php echo $current_step_code; ?>"
	...
	<?php if ($customer->id) { ?>
		<div class="step-customer-logged-in-header-w">
			<div><?php _e('Contact Information', 'latepoint'); ?></div>
			<span><?php _e('Not You?', 'latepoint'); ?></span><a
				data-btn-action="<?php echo OsRouterHelper::build_route_name('auth', 'logout_customer'); ?>" href="#"
				class="step-customer-logout-btn"><?php _e('Logout', 'latepoint'); ?></a>
		</div>
			...
			<?php if (OsSettingsHelper::get_settings_value('steps_hide_login_register_tabs') != 'on') { ?>
				<div class="os-step-tabs">
					<div class="os-step-tab active"
					     data-target=".os-step-new-customer-w"><?php _e('New Customer', 'latepoint'); ?></div>
					<div class="os-step-tab"
					     data-target=".os-step-existing-customer-login-w"><?php _e('Already have an account?', 'latepoint'); ?></div>
				</div>
			<?php } ?>
			...
			<?php if (OsSettingsHelper::get_settings_value('steps_hide_login_register_tabs') != 'on') { ?>
				<div class="os-step-tab-content os-step-existing-customer-login-w" style="display: none;">
					...
					<div class="os-form-buttons os-flex os-space-between">
						<a data-btn-action="<?php echo OsRouterHelper::build_route_name('auth', 'login_customer'); ?>" href="#"
						   class="latepoint-btn latepoint-btn-primary step-login-existing-customer-btn"><?php _e('Log Me In', 'latepoint'); ?></a>
						<a href="#" class="latepoint-btn latepoint-btn-primary latepoint-btn-link step-forgot-password-btn"
						   data-os-action="<?php echo OsRouterHelper::build_route_name('customer_cabinet', 'request_password_reset_token'); ?>"
						   data-os-output-target=".os-password-reset-form-holder"
						   data-os-after-call="latepoint_reset_password_from_booking_init"
						   data-os-params="<?php echo OsUtilHelper::build_os_params(['from_booking' => true]) ?>"><?php _e('Forgot Password?', 'latepoint'); ?></a>
					</div>
				</div>
				...
				<?php if (OsSettingsHelper::is_using_facebook_login()) { ?>
					<div id="facebook-signin-btn"
					     ...
					     class="os-social-login-facebook os-social-login-option"><img
							src="<?php echo LatePoint::images_url() . 'facebook-logo-compact.png' ?>"/><span><?php _e('Login with Facebook', 'latepoint'); ?></span>
					</div>
				<?php } ?>
