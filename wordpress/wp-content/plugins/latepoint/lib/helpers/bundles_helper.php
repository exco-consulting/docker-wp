...
class OsBundlesHelper {
    ...
    ob_start();
		?>
		<div class="summary-box main-box" <?php if($cart_item_id) echo 'data-cart-item-id="'.$cart_item_id.'"'; ?>>
			<?php
			$bundle_headings = [];
			...
			<div class="summary-box-content <?php if($cart_item_id) echo 'os-cart-item'; ?> is-removable">
				...
				<div class="os-remove-item-from-cart"
				     data-confirm-text="<?php _e('Are you sure you want to remove this item from your cart?', 'latepoint'); ?>"
				     data-cart-item-id="<?php echo $cart_item_id; ?>"
				     data-route="<?php echo OsRouterHelper::build_route_name('carts', 'remove_item_from_cart'); ?>"><i
						class="latepoint-icon latepoint-icon-minus"></i></div>
				<?php } ?>
				<div class="sbc-big-item"><?php echo $bundle->name; ?></div>
				<div class="sbc-subtle-item">
					<?php echo implode(', ', $bundle_services_descriptions); ?>
				</div>
			</div>
			<?php if($order_item_id){
				...
				if($user_type == LATEPOINT_USER_TYPE_CUSTOMER){
					...
					echo '<div class="bundle-order-small-info">'.sprintf(__('Order %s', 'latepoint'), '<a href="#" '.OsCustomerHelper::generate_order_summary_btn($order->id).'>#'.$order->confirmation_code.'</a>').'</div>';
				}
				echo '<div class="bookable-items-breakdown">';
					foreach ($bundle_services as $service) {
						...
						if(count($bundle_services) > 1) echo '<div class="bundle-service-info">'.$service->name.' ['.sprintf(__('%d sessions', 'latepoint'), $service->join_attributes['quantity']).']</div>';
						for($i = 0; $i < $service->join_attributes['quantity']; $i++){
							if(isset($bookings[$i])){
								...
								echo '<div class="order-item-variant-bundle-booking is-booked bundle-booking-status-'.esc_attr($bookings[$i]->status).' '.$is_past.'" '.$trigger_html.'>
												...
												<div class="bib-datetime">'.$bookings[$i]->get_nice_start_datetime().'</div>
												<div class="bib-icon"><i class="latepoint-icon latepoint-icon-arrow-right"></i></div>
											</div>';
							}else{
								if($user_type == LATEPOINT_USER_TYPE_CUSTOMER){
									echo '<div class="order-item-variant-bundle-booking os_trigger_booking" data-hide-side-panel="yes" data-hide-summary="yes" data-order-item-id="'.$order_item_id.'" data-selected-service="'.$service->id.'"><div class="booking-item-status-pill"></div><div class="bib-label">'.__('Schedule now', 'latepoint').'</div></div>';
								}else{
									echo '<div class="order-item-variant-bundle-booking" '.OsOrdersHelper::quick_order_btn_html(false, ['order_item_id' => $order_item_id, 'service_id' => $service->id]).'><div class="booking-item-status-pill"></div><div class="bib-label">'.__('Schedule now', 'latepoint').'</div></div>';
								}
							}
						}
                        $cancelled_bookings = OsOrdersHelper::get_bookings_for_order_item($order_item_id, $service->id, [LATEPOINT_BOOKING_STATUS_CANCELLED]);
                        if(!empty($cancelled_bookings)){
                            echo '<div class="order-item-cancelled-bookings-wrapper">';
                                echo '<div class="order-item-cancelled-bookings-heading">'.sprintf(_n('%d Cancelled Appointment', '%d Cancelled Appointments', count($cancelled_bookings), 'latepoint'), count($cancelled_bookings)).'</div>';
                                ...
                                foreach($cancelled_bookings as $cancelled_booking){
                                    echo '<div class="order-item-variant-bundle-booking-wrapper">';
                                        ...
                                        echo '<div class="order-item-variant-bundle-booking is-booked bundle-booking-status-'.esc_attr($cancelled_booking->status).' '.$is_past.'" '.$trigger_html.'>
                                                        <div class="booking-item-status-pill"></div>
                                                        <div class="bib-datetime">'.$cancelled_booking->get_nice_start_datetime().'</div>
                                                        <div class="bib-icon"><i class="latepoint-icon latepoint-icon-arrow-right"></i></div>
                                                    </div>';
                                    echo '</div>';
                                }
				...
				for($i = 1; $i <= $total_bookable_quantity; $i++){
					$is_past = ($i <= $past_count) ? 'is-past' : '';
					$is_booked = ($i <= $booked_count) ? 'is-booked' : '';
					echo '<div class="bookable-item '.$is_past.' '.$is_booked.'"></div>';
				}
				echo '</div>';
