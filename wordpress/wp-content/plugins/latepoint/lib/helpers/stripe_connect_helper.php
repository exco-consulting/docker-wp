...
class OsStripeConnectHelper {
	...
	public static function output_stripe_link_on_customer_quick_form(OsCustomerModel $customer) {
		...
		if ($stripe_customer_id) echo '<div class="payment-processor-customer-link-wrapper">' . __('Stripe Customer', 'latepoint') . '<a target="_blank" href="' . self::build_customer_profile_link($stripe_customer_id, OsSettingsHelper::is_env_payments_dev()) . '">' . __('Open in Stripe', 'latepoint') . '</a></div>';
	}
	...
	public static function add_settings_fields($processor_code) {
		...
		<div class="sub-section-row">
			<div class="sub-section-label">
				<h3><?php _e('Connect (Live)', 'latepoint'); ?></h3>
			</div>
			<div class="sub-section-content">
				<div data-env="<?php echo LATEPOINT_PAYMENTS_ENV_LIVE; ?>"
				     class="payment-processor-connect-status-wrapper stripe-connect-status-wrapper"
				     data-route-name="<?php echo OsRouterHelper::build_route_name('stripe_connect', 'check_connect_status'); ?>">
		...
		<div class="sub-section-row">
			<div class="sub-section-label">
				<h3><?php _e('Connect (Dev)', 'latepoint'); ?></h3>
			</div>
			<div class="sub-section-content">
				<div data-env="<?php echo LATEPOINT_PAYMENTS_ENV_DEV; ?>"
				     class="payment-processor-connect-status-wrapper stripe-connect-status-wrapper"
				     data-route-name="<?php echo OsRouterHelper::build_route_name('stripe_connect', 'check_connect_status'); ?>">
					<div class="os-loading-spinner"></div>
				</div>
	...
	public static function generate_payment_intent_id_and_secret_from_connect(OsOrderIntentModel $order_intent): array {
		...
		if (empty($result['data'])) {
			$error_message = !empty($result['error']) ? __('Payment Error: ' . esc_html($result['error'])) : __('Payment error', 'latepoint');
