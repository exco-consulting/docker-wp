...
class OsEventsHelper {
		...
		for ($day_date = clone $calendar_start; $day_date <= $calendar_end; $day_date->modify('+1 day')) {
			...
			foreach ($day_services_html as $day_service_html) {
				if($daily_count >= 4) {
					...
					data-os-params="' . OsUtilHelper::build_os_params(['target_date_string' => $day_date->format('Y-m-d'), 'filter' => $filter]) . '" 
					data-os-output-target="lightbox" 
					data-os-action="' . OsRouterHelper::build_route_name('events', 'events_day_view') . '">'.sprintf(__('+ %d more', 'latepoint'), count($day_services_html) - 4).'</div>';
					break;
				}
				$daily_count++;
	...
	public static function event_service_box_html(OsServiceModel $service, \LatePoint\Misc\TimePeriod $time_period, array $filter, OsWpDateTime $start_date, string $extra_attrs = ''){
		...
		if(!empty($filter['agent_id'])) $selected_attributes.= ' data-selected-agent="'.$filter['agent_id'].'" ';
		if(!empty($filter['location_id'])) $selected_attributes.= ' data-selected-location="'.$filter['location_id'].'" ';

		if(OsEventsHelper::is_service_slot_passed($service, $start_date, $time_period)){
			$service_box_html.= '<div class="le-service-wrapper is-passed" '.$extra_attrs.'><div class="is-passed-message">'.__('This event has passed', 'latepoint').'</div>';
		}else{
			$start_time_attr = (($time_period->end_time - $time_period->start_time) > $service->duration) ? '' : 'data-selected-start-time="' . $time_period->start_time . '"';
			$service_box_html.= '<div class="le-service-wrapper os_trigger_booking" '.$extra_attrs.' '.$selected_attributes.'
			data-selected-service="' . $service->id . '" 
			data-selected-start-date="' . $start_date->format('Y-m-d') . '" 
			'.$start_time_attr.' 
		...
		$service_box_html.= '<div class="le-service-name">' . $service->name . '</div>';
	...
	public static function events_grid(OsWpDateTime $target_date, array $filter = [], $range_type = 'week', array $restrictions = []): string {
		...
		$html.= '<div class="latepoint-calendar-controls-wrapper">';
			...
			$html.= '<div class="latepoint-calendar-filters">';
				...
				$html.= OsFormHelper::select_field('filter[agent_id]', false, array_merge([['value' => '', 'label' => __('All Agents', 'latepoint')]], OsAgentHelper::get_agents_list()), $clean_filter['agent_id'] ?? '');
				$html.= OsFormHelper::select_field('filter[service_id]', false, array_merge([['value' => '', 'label' => __('All Services', 'latepoint')]], OsServiceHelper::get_services_list()), $clean_filter['service_id'] ?? '');
			$html.= '</div>';
		$html.= '</div>';
