...
class OsFormHelper {
	...
	public static function toggler_group_field( $name, $values, $selected_values = [], $atts = [], $size = 'normal' ) {
		...
		foreach ( $values as $value => $label ) {
			...
			$html         .= '<input type="checkbox" name="' . $name . '[]" value="' . $value . '" ' . $checked_attr . ' ' . self::atts_string_from_array( $atts, [
					'id'    => $id,
	...
	public static function toggler_field( string $name, string $label, bool $is_active, $controlledToggleId = false, $size = false, $atts = [] ): string {
		...
		$instant_update = ! empty( $atts['instant_update_route'] ) ? 'data-os-instant-update="' . $atts['instant_update_route'] . '"' : '';
		$instant_update .= empty( $atts['nonce'] ) ? '' : ' data-nonce="' . $atts['nonce'] . '"';
	...
	public static function textarea_field( $name, $label, $value = '', $atts = array(), $wrapper_atts = array() ) {
		...
		$html        .= '<textarea ' . $validate_html . ' type="text" placeholder="' . esc_attr( $placeholder ) . '" name="' . $name . '" ' . self::atts_string_from_array( $atts, [ 'class' => 'os-form-control' ] ) . '>' . esc_textarea( $value ?? '' ) . '</textarea>';
		$html        .= '</div>';
		$html        .= '</div>';
	...
	public static function select_field( $name, $label, $options = array(), $selected_value = '', $atts = array(), $wrapper_atts = array(), $add_value_if_not_present = false ) {
		...
		if ( isset( $atts['placeholder'] ) && ! empty( $atts['placeholder'] ) ) {
			$html .= '<option value="">' . $atts['placeholder'] . '</option>';
		}
		if ( is_array( $options ) ) {
			foreach ( $options as $key => $option ) {
				if ( isset( $option['value'] ) && isset( $option['label'] ) ) {
		...
		if ( $add_value_if_not_present && ! isset( $options[ $selected_value ] ) ) {
			$html .= '<option value="' . $selected_value . '">' . $selected_value . '</option>';
		}
		$html .= '</select>';
		$html .= '</div>';
		if ( ! empty( $wrapper_atts ) ) {
	...
	public static function time_field( $name, $label, $value = '', $as_period = false ) {
		...
		if ( $label ) {
			$html .= '<label for="' . $name . '[formatted_value]">' . $label . '</label>';
		}
	...
	public static function color_picker( $name, $label, $value = '', $atts = array(), $wrapper_atts = array() ) {
		...
		$html .= '<input maxlength="7" type="text" name="' . $name . '" placeholder="' . __( 'Pick a color', 'latepoint' ) . '" value="' . $value . '" ' . self::atts_string_from_array( $atts, [ 'class' => 'os-form-control' ] ) . '/>';
		$html .= '</div>';
		$html .= '</div>';
		if ( ! empty( $wrapper_atts ) ) {
			$html .= '</div>';
	...
	public static function checkbox_field( $name, $label, $value = '', $is_checked = false, $atts = array(), $wrapper_atts = array(), $off_value = 'off' ) {
		...
		if ( $label ) {
			$html .= '<label for="' . $atts['id'] . '">';
		}
		if ( $off_value !== false ) {
			$html .= '<input type="hidden" name="' . $name . '" value="' . $off_value . '"/>';
		}
		$html .= '<input type="checkbox" '.$validate_html.' name="' . $name . '" value="' . $value . '" ' . $checked_attr . ' ' . self::atts_string_from_array( $atts, [ 'class' => 'os-form-checkbox' ] ) . '/>';
		if ( $label ) {
			$html .= $label . '</label>';
		}
		$html .= '</div>';
	...
	public static function quantity_field( $name, $placeholder, $value = '' ): string {
      ...
      <input type="text" name="' . $name . '" class="item-quantity-selector-input" value="' . $value . '" placeholder="' . $placeholder . '">
