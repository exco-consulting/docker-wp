...
class OsShortcodesHelper {
	...
	public static function shortcode_latepoint_resources( $atts ) {
		...
		if ( $atts['button_bg_color'] ) {
			$button_style .= 'background-color: ' . $atts['button_bg_color'] . '!important;';
		}
		if ( $atts['button_text_color'] ) {
			$button_style .= 'color: ' . $atts['button_text_color'] . '!important;';
		}
		if ( $atts['button_font_size'] ) {
			$button_style .= 'font-size: ' . $atts['button_font_size'] . '!important;';
		}
		if ( $atts['button_border_radius'] ) {
			$button_style .= 'border-radius: ' . $atts['button_border_radius'] . '!important;';
		}
	...
	public static function shortcode_latepoint_book_form( $atts, $content = "" ) {
		...
		if ( $atts['show_service_categories'] ) {
			$restrictions['show_service_categories'] = $atts['show_service_categories'];
		}
		if ( $atts['calendar_start_date'] ) {
			$restrictions['calendar_start_date'] = $atts['calendar_start_date'];
		}

		$presets = [];
		if ( $atts['selected_location'] ) {
			$presets['selected_location'] = $atts['selected_location'];
		}
		if ( $atts['selected_agent'] ) {
			$presets['selected_agent'] = $atts['selected_agent'];
		}
		if ( $atts['selected_service'] ) {
			$presets['selected_service'] = $atts['selected_service'];
		}
		if ( $atts['selected_duration'] ) {
			$presets['selected_duration'] = $atts['selected_duration'];
		}
		if ( $atts['selected_total_attendees'] ) {
			$presets['selected_total_attendees'] = $atts['selected_total_attendees'];
		}
		if ( $atts['selected_service_category'] ) {
			$presets['selected_service_category'] = $atts['selected_service_category'];
		}
		if ( $atts['selected_start_date'] ) {
			$presets['selected_start_date'] = $atts['selected_start_date'];
		}
		if ( $atts['selected_start_time'] ) {
			$presets['selected_start_time'] = $atts['selected_start_time'];
		}
		if ( $atts['source_id'] ) {
			$presets['source_id'] = $atts['source_id'];
		}


		$booking_form_styles = [];
		if($atts['hide_side_panel'] == 'yes'){
			$booking_form_styles['hide_side_panel'] = true;
		}

		if($atts['hide_summary'] == 'yes'){
			$booking_form_styles['hide_summary'] = true;
		}

		$steps_controller = new OsStepsController();
		$output           = $steps_controller->start( $restrictions, $presets, $booking_form_styles, false, 'inline_form' );
