...
class OsStepsHelper {
	...
	public static function output_step_edit_form( $step ) {
		...
		?>
        <div class="step-w" data-step-code="<?php echo $step->code; ?>"
             data-step-order-number="<?php echo $step->order_number; ?>">
            <div class="step-head">
                <div
                        class="step-drag <?php echo ( $can_reorder ) ? '' : 'disabled'; ?>"><?php if ( ! $can_reorder ) {
						echo '<span>' . __( 'Order of this step can not be changed.', 'latepoint' ) . '</span>';
					} ?></div>
                <div class="step-code"><?php echo $step->title; ?></div>
                <div class="step-type"><?php echo str_replace( '_', ' ', $step->code ); ?></div>
				<?php if ( $step->code == 'locations' && ( OsLocationHelper::count_locations() <= 1 ) ) { ?>
                    <a href="<?php echo OsRouterHelper::build_link( OsRouterHelper::build_route_name( 'locations', 'index' ) ); ?>"
                       class="step-message"><?php _e( 'Since you only have one location, this step will be skipped', 'latepoint' ); ?></a>
				<?php } ?>
				<?php if ( $step->code == 'payment' && ! OsPaymentsHelper::is_accepting_payments() ) { ?>
                    <a href="<?php echo OsRouterHelper::build_link( OsRouterHelper::build_route_name( 'settings', 'payments' ) ); ?>"
                       class="step-message"><?php _e( 'Payment processing is disabled. Click to setup.', 'latepoint' ); ?></a>
				<?php } ?>
            ...
            <div class="step-body">
                <div class="os-form-w">
                    <form data-os-action="<?php echo OsRouterHelper::build_route_name( 'settings', 'update_step' ); ?>"
                          action="">
                        ...
                        <div class="sub-section-row">
                            ...
                            <div class="sub-section-content">
                                ...
                                <div id="custom-step-image-w-<?php echo $step->code; ?>"
                                     class="custom-step-image-w-<?php echo $step->code; ?>"
                        ...
                        <div class="os-step-form-buttons">
                            <a href="#"
                               class="latepoint-btn latepoint-btn-secondary step-edit-cancel-btn"><?php _e( 'Cancel', 'latepoint' ); ?></a>
...
//		echo OsSettingsHelper::read_encoded('');
		...
		foreach ( $steps as $step_code ) {
			...
			if ( isset( $rules[ $rule_step_code ]['after'] ) ) {
				...
				if ( $after_index === false || $after_index >= $current_index ) {
					$errors[] = sprintf( __( 'Step "%s" has to come after "%s"' ), self::get_step_label_by_code( $rule_step_code ), self::get_step_label_by_code( $rules[ $rule_step_code ]['after'], $parent_code ) );
				}
			}
			...
			if ( isset( $rules[ $rule_step_code ]['before'] ) ) {
				...
				if ( $before_index === false || $before_index <= $current_index ) {
					$errors[] = sprintf( __( 'Step "%s" has to come before "%s"' ), self::get_step_label_by_code( $rule_step_code ), self::get_step_label_by_code( $rules[ $rule_step_code ]['before'], $parent_code ) );
				}
			}
		}
		...
		switch ( $selected_step_code ) {
			...
			case 'booking__datepicker':
				...
				if ( $service ) {
					...
					?>
                    <div class="os-dates-w" data-time-pick-style="<?php echo OsStepsHelper::get_time_pick_style(); ?>">
                    ...
                    <div
                            class="time-selector-w <?php echo 'time-system-' . OsTimeHelper::get_time_system(); ?> <?php echo ( OsSettingsHelper::is_on( 'show_booking_end_time' ) ) ? 'with-end-time' : 'without-end-time'; ?> style-<?php echo OsStepsHelper::get_time_pick_style(); ?>">
			...
			case 'payment__methods':
			case 'payment__pay':
				echo '<div class="booking-preview-step-skipped-message">' . __( "Step is skipped until you install a payment addon.", 'latepoint' ) . '</div>';
				break;
		}
	...
	public static function set_active_cart_item_object( array $cart_item_params = [] ): OsCartItemModel {
		...
		if ( self::$active_cart_item->is_bundle() ) {
			...
			self::$active_cart_item->item_data = is_array( $cart_item_params['item_data'] ) ? json_encode( $cart_item_params['item_data'], true ) : $cart_item_params['item_data'];
		} else {
			// booking gets data from booking object
			self::$active_cart_item->item_data = json_encode( self::$booking_object->generate_params_for_booking_form(), true );
		}
