...
class OsOrderIntentModel extends OsModel {
	...
	public function set_payment_data_value( string $key, string $value, bool $save = true ) {
		...
		$this->payment_data             = json_encode( $this->payment_data_arr );
		if ( $save ) {
	...
	public function convert_to_order() {
		...
		try {
			...
			$order->price_breakdown = json_encode( $cart_from_intent->generate_price_breakdown_rows(['balance', 'total', 'subtotal']));
				...
				if ( $order_bookings ) {
					foreach ( $order_bookings as $order_item_id => $order_booking ) {
						...
						if ( $order_booking->save() ) {
							...
							$order_item->update_attributes( [ 'item_data' => json_encode( $item_data ) ] );
						} else {
