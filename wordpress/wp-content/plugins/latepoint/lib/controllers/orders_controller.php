...
if ( ! class_exists( 'OsOrdersController' ) ) :
	...
	class OsOrdersController extends OsController {
		...
		public function view_order_log() {
			...
			$activities = $activities->where( [ 'order_id' => $this->params['order_id'] ] )->order_by( 'id desc' )->get_results_as_models();
		...
		public function create_or_update() {
			...
			if ( $customer->save() ) {
				...
				$this->send_json( [
					...
					'message' => __( 'Error: ' . implode( ', ', $customer->get_error_messages() ), 'latepoint' ),
					]
				);
			}
			...
			if($validation_errors){
				$this->send_json( array( 'status' => LATEPOINT_STATUS_ERROR, 'message' => __( 'Error: ' . implode( ', ', $validation_errors ), 'latepoint' ) ) );
			}
			...
			if ( ! empty( $this->params['price_breakdown'] ) ) {
				$order->price_breakdown = json_encode( OsOrdersHelper::generate_price_breakdown_from_params($this->params['price_breakdown']) );
			}
			...
			if ( $order->save() ) {
				...
				$status        = LATEPOINT_STATUS_ERROR;
				$response_html = __( 'Error: ' . implode( ', ', $order->get_error_messages() ), 'latepoint' );
			}
