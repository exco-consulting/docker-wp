...
class OsFeatureCustomFieldsHelper {
  ...
  public static function validate_custom_fields($model, $alternative_validation = false, $skip_properties = []){
	...
	public static function extract_file_properties_by_custom_field_id(string $file_property, OsModel $model, string $custom_field_id) : array{
		if($model instanceof OsBookingModel){
			$files = OsParamsHelper::get_file('bookings');
			...
			$form_id = empty($model->form_id) ? $model->id : $model->form_id;
			return $files[$file_property][$form_id]['custom_fields'][$custom_field_id] ?? [];
		}else{
    ...
    if(($model instanceof OsBookingModel) || ($model instanceof OsCustomerModel)){
      ...
      if($custom_fields_structure){
        foreach($custom_fields_structure as $custom_field){
					switch($custom_field['type']){
						case 'file_upload':
							if(!empty(self::extract_file_properties_by_custom_field_id('name', $model, $custom_field['id']))){
								if ( ! function_exists( 'wp_handle_upload' ) ) require_once( ABSPATH . 'wp-admin/includes/file.php' );
								for($i = 0; $i<count(self::extract_file_properties_by_custom_field_id('name', $model, $custom_field['id'])); $i++){
									$file = [
										'name' => self::extract_file_properties_by_custom_field_id('name', $model, $custom_field['id'])[$i],
										'type' => self::extract_file_properties_by_custom_field_id('type', $model, $custom_field['id'])[$i],
										'tmp_name' => self::extract_file_properties_by_custom_field_id('tmp_name', $model, $custom_field['id'])[$i],
										'error' => self::extract_file_properties_by_custom_field_id('error', $model, $custom_field['id'])[$i],
										'size' => self::extract_file_properties_by_custom_field_id('size', $model, $custom_field['id'])[$i]
									];
