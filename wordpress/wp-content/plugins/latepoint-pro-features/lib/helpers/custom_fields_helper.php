...
class OsCustomFieldsHelper {
  ...
  public static function prepare_to_save($array_to_filter){
	...
	public static function get_custom_field_types_with_default_value(){
		 ...
		 */
		return apply_filters('latepoint_custom_field_types_with_default_value', ['text', 'phone_number', 'number', 'hidden', 'select', 'textarea']);
	}
	 ...
	 * @return string
	 */
	public static function output_custom_fields_for_model($custom_fields, OsModel $model, $fields_for = 'customer', $name_prefix = ''){
		$html = '';
		if(empty($custom_fields)) return $html;
		$name_prefix = empty($name_prefix) ? $fields_for : $name_prefix;
    foreach($custom_fields as $custom_field){
	    if (OsAuthHelper::get_highest_current_user_type() == 'customer' && $custom_field['visibility'] != 'public') continue;
      ...
      switch ($custom_field['type']) {
        case 'text':
          $html.= OsFormHelper::text_field($name, $label, $value, ['class' => $required_class, 'placeholder' => $placeholder, 'validate' => $validate, 'theme' => 'simple'], ['class' => $width]);
          break;
        case 'phone_number':
          $html.= OsFormHelper::phone_number_field($name, $label, $value, ['class' => $required_class, 'placeholder' => $placeholder, 'validate' => $validate, 'theme' => 'simple'], ['class' => $width]);
          break;
        case 'number':
			// TODO: Implement min/max in CF settings
          $html.= OsFormHelper::number_field($name, $label, $value, null, null, ['class' => $required_class, 'placeholder' => $placeholder, 'validate' => $validate, 'theme' => 'simple'], ['class' => $width]);
          break;
        case 'hidden':
          $html.= OsFormHelper::hidden_field($name, $value);
          break;
        case 'textarea':
          $html.= OsFormHelper::textarea_field($name, $label, $value, ['class' => $required_class, 'placeholder' => $placeholder, 'validate' => $validate, 'theme' => 'simple'], ['class' => $width]);
          break;
        case 'select':
          $html.= OsFormHelper::select_field($name, $label, OsFormHelper::generate_select_options_from_custom_field($custom_field['options']), $value, ['class' => $required_class, 'placeholder' => $placeholder, 'validate' => $validate], ['class' => $width]);
          break;
        case 'checkbox':
        ...
        case 'google_address_autocomplete':
          $html.= OsFormHelper::text_field($name, $label, $value, ['class' => $required_class.' latepoint-google-places-autocomplete', 'placeholder' => $placeholder, 'validate' => $validate, 'theme' => 'simple'], ['class' => $width]);
          break;
	      case 'file_upload':
					...
					if($model instanceof OsBookingModel){
						...
						$override_name = 'bookings['.$model->get_form_id().'][custom_fields]['.$custom_field['id'].']';
  ...
  public static function has_validation_errors($custom_field){
  	$errors = [];
  	if(empty($custom_field['label'])) $errors[] = __('Field Label can not be empty', 'latepoint-pro-features');
  	if(empty($custom_field['type'])){
  		$errors[] = __('Type can not be empty', 'latepoint-pro-features');
  	}else{
  		if($custom_field['type'] == 'select'){
		  	if(empty($custom_field['options'])) $errors[] = __('Options for select box can not be blank', 'latepoint-pro-features');
  		}
  	}
  	if(empty($errors)){
  		return false;
  ...
  public static function validate_fields($fields_data, $fields_rules, $fields_for){
    $errors = [];
    foreach($fields_rules as $custom_field){
      if($custom_field['required'] != 'on') continue;
			switch($custom_field['type']) {
				case 'checkbox':
	        ...
	        if (empty($fields_data[$custom_field['id']]) || $fields_data[$custom_field['id']] == 'off') {
				...
				case 'file_upload':
					...
					if(empty($fields_data[$custom_field['id']]) && empty($files['name']['custom_fields'][$custom_field['id']])){
  ...
  public static function generate_condition_form($custom_field_id, $condition_id = false, $condition_data = false){
    $properties = [ 'service' => __('Service', 'latepoint-pro-features'),
    ...
    if(!$condition_id){
      ...
      $condition_data = [  'property' => 'service', 
                           'operator' => false, 
                           'value' => false];
    }
    $html = '<div class="cf-condition" data-condition-id="'.$condition_id.'">'.
                ...
                OsFormHelper::select_field('custom_fields['.$custom_field_id.'][conditions]['.$condition_id.'][property]', false, $properties, $condition_data['property'], ['class' => 'custom-field-condition-property', 'data-route' => OsRouterHelper::build_route_name('custom_fields', 'available_values_for_condition_property')], ['class' => 'custom-field-condition-property-w']).
                OsFormHelper::select_field('custom_fields['.$custom_field_id.'][conditions]['.$condition_id.'][operator]', false, array('equal' => __('is equal to', 'latepoint-pro-features'), 'not_equal' => __('is not equal to', 'latepoint-pro-features')), $condition_data['operator'], [], ['class' => 'custom-field-condition-operator-w']).
    ...
    if($custom_fields){
      ...
      if($custom_fields_arr && is_array($visibility)){
        foreach($custom_fields_arr as $id => $custom_field){
          if(!isset($custom_field['conditions'])) $custom_field['conditions'] = [];
          if(!isset($custom_field['hide_on_summary'])) $custom_field['hide_on_summary'] = 'off';
          if(!isset($custom_field['conditional'])) $custom_field['conditional'] = 'off';
					if($booking_object && !empty($booking_object->custom_fields) && !empty($booking_object->custom_fields[$custom_field['id']])) $custom_field['value'] = $booking_object->custom_fields[$custom_field['id']];
          // if either booking object is not set, or condition when to show is satisfied - show it
          if(!$booking_object || (self::is_condition_satisfied($custom_field, $booking_object)) && (!isset($custom_field['visibility']) || in_array($custom_field['visibility'], $visibility))) $visible_fields[$id] = $custom_field;
        }
      }
	  if (!empty($atts)) {
  ...
  public static function is_condition_satisfied($custom_field, $booking_object){
    // no conditions
    if(!$custom_field['conditions'] || $custom_field['conditional'] == 'off') return true;
    foreach($custom_field['conditions'] as $condition){
      ...
      // not satisfied
	    switch($condition['operator']){
		    case 'not_equal':
		      if(in_array($current_value, $required_values)) return false;
				break;
		    default:
		      if(!in_array($current_value, $required_values)) return false;
				break;
