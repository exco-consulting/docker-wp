<form data-os-form-block-id="<?php echo $custom_field['id']; ?>"
	...
	<div class="os-form-block-i">
		<div class="os-form-block-header <?php if($custom_field['required'] == 'on') echo 'os-form-block-required'; ?>">
			...
			<div class="os-form-block-name"><?php echo !empty($custom_field['label']) ? $custom_field['label'] : __('New Field', 'latepoint-pro-features'); ?></div>
        ...
        <div class="sub-section-content">
				  <?php echo OsFormHelper::select_field('custom_fields['.$custom_field['id'].'][type]', 
					...
					<div class="custom-fields-google-places-api-status" <?php if($custom_field['type'] != 'google_address_autocomplete') echo 'style="display: none;"'; ?>">
						<?php if(empty(OsSettingsHelper::get_settings_value('google_places_api_key'))) echo '<div class="latepoint-message latepoint-message-warning mt-1">'.__('To use address field, you need to enter Google API key in general settings.', 'latepoint-pro-features').'</div>'; ?>
					</div>
        </div>
      </div>

      <div class="sub-section-row custom-fields-select-values" <?php if($custom_field['type'] != 'select') echo 'style="display: none;"'; ?>>
        <div class="sub-section-label">
